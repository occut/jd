                    ...
                    <div class="box">
                        <div class="box-header">
                            <!--<h3 class="box-title"><a href="javascript:;" onclick="add('导出账号','<?php echo U('Wechat/WeExport',array('wei_static'=>$static));?>')"><button type="button" class="btn btn-block btn-primary">导出账号</button></a></h3>-->
                             <h3 class="box-title"><a href="javascript:;" onclick="add('导入账户地址','<?php echo U('Wechat/idcardImport',array('wei_static'=>$static));?>')"><button type="button" class="btn layui-btn box-title" style="font-size: 14px;margin: 10px 5px;">导入账号</button></a></h3>
                            <h3 class="box-title"><button class="btn layui-btn box-title" style="font-size: 14px;margin: 10px 5px;" onclick="delAll('<?php echo U('Wechat/deletecard');?>')">批量删除</button></h3>
                            <h3 class="box-title"><a class="btn layui-btn box-title" style="font-size: 14px;margin: 10px 5px;" href="<?php echo U('Tasks/addrSearch');?>">搜索</a></h3>
                            <form method="post" style="float: left;" action="<?php echo U('Wechat/WechaIdCard');?>">
                                <div class="box-header">
                                    <input class="layui-input" name="product" value="<?php echo session('card_city');?>" placeholder="市" style="float:left;width:140px;margin-left:10px;">
                        ...
                        <div class="box-body">
                            <table id="example2" class="table table-bordered table-hover" style = "text-align: center;">
                                ...
                                <?php if(is_array($usergroups)): $i = 0; $__LIST__ = $usergroups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tnd">
                                        <td><input type="checkbox" value="<?php echo ($vo["card_id"]); ?>" name="delAll"></td>
                                        <td><?php echo ($vo["card_id"]); ?></td>
                                        <td><?php echo ($vo["card_name"]); ?></td>
                                        <td><?php echo ($vo["card_product"]); ?></td>
                                        <td><?php echo ($vo["card_city"]); ?></td>
                                        <td><?php echo ($vo["card_area"]); ?></td>
                                        <td><?php echo ($vo["card_path"]); ?></td>
