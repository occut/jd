...
class ShopController extends SuperController {
  public function Shops(){
    ...
    $total=$WechaModel->countShops($a);
    $page = new \Org\Page\Page($total, 100);
    $usergroups=$WechaModel->selectShopsByPage($a,$page);
    $show = $page->show();
  ...
  public function deleteshop(){
    ...
    public function showShopDetails(){
     $WechaModel = D('Shop');
        ...
        $con = $Management->where($data)->select();
        $this->assign('con',$con);
    ...
    public function addshop(){
        ...
        $this->assign('con',$con);
        $this->display('Shop/addshop');//将值显示在静态页面上
    }
    //添加店铺
    ...
    public function shopSearch(){
      $WechaModel = D('Shop');
      $sh['ag_id'] = session('AgentId');
      $a['agent_id'] = session('AgentId');
      ...
      if(empty($id)){
            $shop = $WechaModel->where($sh)->order('id')->find();
    ...
    $show = $page->show();
    $val = $WechaModel->where($sh)->order('id')->select();
    //赋值到模版
    $this->assign('id',$id);
...
// 将订单详情显示出来
    ...
    public function exportshop(){
        ...
        $objExcel->getActiveSheet()->setCellValue('F1','订单号');
        $objExcel->getActiveSheet()->setCellValue('G1','SKU');
        $objExcel->getActiveSheet()->setCellValue('H1','关键词');
        $objExcel->getActiveSheet()->setCellValue('I1','金额');
        $objExcel->getActiveSheet()->setCellValue('J1','物流单号');
        $objExcel->getActiveSheet()->setCellValue('K1','物流');
        $objExcel->getActiveSheet()->setCellValue('L1','下单时间');
        $objExcel->getActiveSheet()->setCellValue('M1','下单端口');
        $objExcel->getActiveSheet()->setCellValue('N1','收货评语');
        $objExcel->getActiveSheet()->setCellValue('O1','任务进度');
        $objExcel->getActiveSheet()->setCellValue('P1','店铺名');
        $objExcel->getActiveSheet()->setCellValue('Q1','收货时间');
        $objExcel->getActiveSheet()->setCellValue('R1','完成设备');
        ...
        for ($i = 2; $i <= $count+1; $i++) {
            ...
            $objExcel->getActiveSheet()->setCellValue('F' . $i, $Wecha[$i-2]['we_order']);
            $objExcel->getActiveSheet()->setCellValue('G' . $i, $Wecha[$i-2]['we_SKU']);
            $objExcel->getActiveSheet()->setCellValue('H' . $i, $Wecha[$i-2]['we_word']);
            $objExcel->getActiveSheet()->setCellValue('I' . $i, $Wecha[$i-2]['wei_gold']);
            $objExcel->getActiveSheet()->setCellValue('J' . $i, $Wecha[$i-2]['we_oddnumbers']);
            $objExcel->getActiveSheet()->setCellValue('K' . $i, $Wecha[$i-2]['we_logistics']);
            $objExcel->getActiveSheet()->setCellValue('L' . $i, $Wecha[$i-2]['bought_time']);
            $objExcel->getActiveSheet()->setCellValue('M' . $i, $Wecha[$i-2]['we_port']);
            $objExcel->getActiveSheet()->setCellValue('N' . $i, $Wecha[$i-2]['comment']);
            $objExcel->getActiveSheet()->setCellValue('O' . $i, $Wecha[$i-2]['wei_remarks']);
            $objExcel->getActiveSheet()->setCellValue('P' . $i, $Wecha[$i-2]['shop_name']);
            $objExcel->getActiveSheet()->setCellValue('Q' . $i, $Wecha[$i-2]['receipt_time']);
            $objExcel->getActiveSheet()->setCellValue('R' . $i, $Wecha[$i-2]['we_we']);
        }
