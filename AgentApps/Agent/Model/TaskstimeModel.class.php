...
//    }
    ...
    public function selectTasksTotalSize($data) {
        $result = M('Taskstime')->where($data)->count();
        return $result;
    }
     ...
     * 分页数据集
     */
    public function selectTasksByPage($data,$Page) {
            $result = M('Taskstime')->where($data)->order('time_endtime asc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
            return $result;
    }
