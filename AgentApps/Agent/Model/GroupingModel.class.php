...
class GroupingModel extends Model {
     ...
     * 获取用户分组表中的总条数
     */
    public function selectUserGroupTotalSize($data) {
        $result = M('grouping')->where($data)->count();
        return $result;
    }
     ...
     * 分页数据集
     */
    public function selectGroupingByPage($data,$Page) {

            $result = M('Grouping')->where($data)->limit($Page->firstRow . ',' . $Page->listRows)->select();
            return $result;
