...
class SellerBalanceModel extends Model {

    /**
     * 获取文章表中的总条数
     */
    static public function selectSize($where) {
        $result = M('SellerBalance')->where($where)->count();
        return $result;
     ...
     */
     static public function selectByPages($where,$page){
        $result = M('SellerBalance')->where($where)->limit($page->firstRow . ',' . $page->listRows)->order('ag_ltime desc')->select();
        return $result;
    }
    /*
     * 添加数据
