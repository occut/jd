...
//        $tmp .= '<td>'.$v['module_name'].'</td>';
        $tmp .= '<td>';
        if($v['fid']==0){
            $tmp .= '<a href="'.U('Agent/addChildModule',array('moduleid'=>$v['module_id'])).'" style="margin-right:5px">添加</a>';
        }
        $tmp .= '<a href="'.U('Agent/editModule',array('moduleid'=>$v['module_id'])).'" style="margin-right:5px">编辑</a>';
        $tmp .= '<a href="'.U('Agent/deleteModule',array('moduleid'=>$v['module_id'])).'">删除</a>';
        $tmp .= '</td>';
        $tmp .= '</tr>';
    });
    return $tmp;
...
function getordcode(){
 ...
 function gets($url, $data_type='text',$USERPWD=null) {
    $cl = curl_init();
    if(stripos($url, 'https://') !== FALSE) {
        curl_setopt($cl, CURLOPT_SSL_VERIFYPEER, FALSE);
        curl_setopt($cl, CURLOPT_SSL_VERIFYHOST, FALSE);
...
function selectpayment($id){
    $PaymentModel = D('Management');
    $con = $PaymentModel->where(['ag_id'=>$id])->find();
    return $con;
}
function selectagent($id){
    $PaymentModel = D('Agent');
    $con = $PaymentModel->where(['ag_id'=>$id])->find();
    return $con;
}
