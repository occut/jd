                        ...
                        <div class="box-body">
                            <table id="example2" class="table table-bordered table-hover layui-table" style = "text-align: center;">
                                ...
                                <?php if(is_array($tasks)): $i = 0; $__LIST__ = $tasks;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tnd">
                                        ...
                                        <td style="text-align: center;"><span><?php echo selectpayment($vo['seller_id'])['ag_name'];?></span></td>
                                        <td style="text-align: center;"><span><?php echo ($vo["addtasktime"]); ?></span></td>
                                        <td style="text-align: center;"><span><?php echo ($vo["shop_name"]); ?></span></td>
                                        <td style="text-align: center;"><span><?php echo ($vo["allmoney"]); ?></span></td>
                                        <td style="text-align: center;"><span><?php echo ($vo["order_num"]); ?>/<?php echo ($vo["process"]); ?></span></td>
                                        <td style="text-align: center;"><a href="javascript:;" onclick="add('查看任务','<?php echo U('Tasks/viewtask',array('id'=>$vo['task_id']));?>')">查看</a></td>
                                        <td style="text-align: center;" id="examine" >
                                            <a href="javascript:if(confirm('是否通过审核?')) location='<?php echo U('Tasks/Taskaudit',array('id'=>$vo['task_id']));?>'">
                                        ...
                                        <td style="text-align: center;">
                                            ...
                                            <a href="javascript:if(confirm('确实要删除吗?')) location='<?php echo U('Tasks/tasksdell',array('task_id'=>$vo['task_id']));?>'" style="display: <?php if($vo["status"] == 0): ?>none<?php else: ?>block<?php endif; ?>">取消任务</a>
                                        </td>
                                        <td style="text-align: center;"><a href="<?php echo U('Tasks/WeExport',array('id'=>$vo['task_id']));?>">下载</a></td>

                                        <td style="text-align: center;">

                                            <?php if($vo['payway'] == 0): ?>已结算
                                                <?php else: ?>
                                                <a href="javascript:if(confirm('确实要结算么吗?')) location='<?php echo U('Tasks/settlement',array('id'=>$vo['task_id']));?>'" style="color: red;">未结算</a><?php endif; ?>
                                        </td>
