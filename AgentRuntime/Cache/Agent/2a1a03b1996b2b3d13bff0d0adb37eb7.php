                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <a href="<?php echo U('Agent/addmanagement');?>" class="layui-btn ">添加商家</a>
                        </div>
                        <!-- /.box-header -->
                        <div class="box-body">
                            <table id="example2" class="table table-bordered table-hover layui-table" style = "text-align: center;">
                                ...
                                <?php if(is_array($Agent)): $i = 0; $__LIST__ = $Agent;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tnd">
                                        ...
                                        <td><?php echo ($vo['ag_time']); ?></td>
                                        <td><?php echo ($vo['ag_ltime']); ?></td>
                                        <td><a href="javascript:if(confirm('确实要更改状态吗?'))location='<?php echo U('Agent/modifystate',array('id'=>$vo['ag_id']));?>'"><?php echo ($vo['ag_ban']==1 ?'启用':'禁用'); ?></a></td>
                                        <td><a href="<?php echo U('Agent/ModifyPassword',['id'=>$vo['ag_id']]);?>">修改密码</a></td>
                                        <td><a href="<?php echo U('Agent/selectmanagement',['id'=>$vo['ag_id']]);?>">查看</a></td>
                                        <td><a href="<?php echo U('Agent/selectaddress',['id'=>$vo['ag_id']]);?>">地址</a></td>
