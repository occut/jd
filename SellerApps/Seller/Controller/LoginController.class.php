...
class LoginController extends Controller {
    ...
    public function index() {
        $a = session('SellerId');
        if(!empty($a)){
            redirect('/Seller.php/Index/', 1, '页面跳转中...');
        }
...
//            dump(admin_md5($adminPass) != $admin['seller_password']);die;
            ...
            D('Management')->saveAgent($adminId, $data);
            //用户名和用户id保存到SESSION
            session('SellerId', $admin['ag_id']);
            session('SellerName', $admin['ag_name']);
