...
//    根据商家id查找店铺
    public function selectShopBySeller($sellerid){
        $result = M('shop')->where('seller_id ='.$sellerid)->select();
        return $result;
    }
    /**
     *根据店铺的id来删除企业店铺
     ...
     * 获取企业店铺总数
     */
    public function countShops($data){
        $result = M('shop')->where($data)->count();
        return $result;
    }
     ...
     * 获取每页的数据集
     */
    public function selectShopsByPage($data,$page){
        $result = M('shop')->where($data)->limit($page->firstRow . ',' . $page->listRows)->order('id desc')->select();
        return $result;
    }
