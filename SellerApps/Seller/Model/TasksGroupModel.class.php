...
class TasksGroupModel extends Model {
     ...
     * 获取任务分组表中的总条数
     */
    public function selectTasksGroupTotalSize($data) {
        $result = M('tasksGroup')->where($data)->count();
        return $result;
    }
     ...
     * 分页数据集
     */
    public function selectTasksGroupByPage($data,$Page) {


            $result = M('tasksGroup')->where($data)->limit($Page->firstRow . ',' . $Page->listRows)->select();
            return $result;
